...
func (d *Deliverer) fixedDestination(protocol string) string {
	if protocol == config.ProtocolLMTP {
		return d.config.DefaultLMTPDestination
	}
	return d.config.DefaultSMTPDestination
}
...
func (d *Deliverer) dialAndHello(ctx context.Context, logger *slog.Logger, traceID string, mxHost, sourceIP string, preferIPv6 bool, protocol string) (*dialResult, DeliveryResult, error) {
	...
	case result := <-resultCh:
		if result.err != nil {
			...
			// If the goroutine consumed (closed) conn, mark it so the
			// deferred cleanup won't double-close.
			connConsumed = true
