...
func TestNewSRS_Success(t *testing.T) {
	srs, err := NewSRS("example.com", "my-secret-key-123", 21, 4, "=", false)
	if err != nil {
		t.Fatalf("NewSRS failed: %v", err)
	}
	if srs.domain != "example.com" {
		t.Errorf("Expected domain 'example.com', got '%s'", srs.domain)
	}
	if srs.maxAge != 21 {
...
func TestNewSRS_ShortSecret(t *testing.T) {
	_, err := NewSRS("example.com", "short", 21, 4, "=", false)
	if err == nil {
...
func TestNewSRS_InvalidHashLength(t *testing.T) {
	...
	for _, hashLen := range tests {
		_, err := NewSRS("example.com", "my-secret-key-123", 21, hashLen, "=", false)
		if err == nil {
...
func TestForward_Basic(t *testing.T) {
	srs, err := NewSRS("forwarding.com", "my-secret-key-123456", 21, 4, "=", false)
	if err != nil {
		t.Fatalf("NewSRS failed: %v", err)
	}
...
func TestForward_EmptyAddress(t *testing.T) {
	srs, _ := NewSRS("forwarding.com", "my-secret-key-123456", 21, 4, "=", false)
...
func TestForward_SRS0ToSRS1(t *testing.T) {
	srs, _ := NewSRS("forwarding2.com", "my-secret-key-123456", 21, 4, "=", false)
...
func TestGenerateHash_Length(t *testing.T) {
	...
	for _, hashLen := range tests {
		srs, _ := NewSRS("forwarding.com", "my-secret-key-123456", 21, hashLen, "=", false)
...
func TestDifferentSecrets_DifferentHashes(t *testing.T) {
	srs1, _ := NewSRS("forwarding.com", "secret-key-number-1", 21, 4, "=", false)
	srs2, _ := NewSRS("forwarding.com", "secret-key-number-2", 21, 4, "=", false)
